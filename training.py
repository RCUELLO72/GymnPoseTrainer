...
class GymnDataset(Dataset):
    ...
    def __init__(self,data_df,data,transform=None):
        self.df = data_df
        self.data = data['data']
...
def getDataLoaders(data,batch_size,transforms,test_ratio=0.25):
    X_train, X_test = getBalancedList(data,test_ratio)
    trainData = GymnDataset(X_train,data,transforms)
    testData = GymnDataset(X_test,data,transforms)
...
class GymnModel(nn.Module):
    def __init__(self):
        ...
        self.mxpool = nn.MaxPool2d( kernel_size=(3, 3))
        self.fc1 = nn.Linear(2720,100)
        self.fc2 = nn.Linear(100,11)
        self.dpout = nn.Dropout()
    
    def forward(self,x):
        x = F.relu(self.mxpool(self.conv1(x)))
        x = F.relu(self.mxpool(x))
        x = x.view(-1,2720)
        x = F.relu(self.fc1(x))
        x = self.dpout(x)
        x = F.relu(self.fc2(x))
